...
FORMULARIO = "REPORTE DE VISITA A CENTRO DE TRABAJO"
...
NOMBRE_ARCHIVO_FIJO = "datos_visitas.csv"
...
def limpiar_columna_extra(df):
    """
    Elimina columnas fantasma que genera el webservice (ej. Column17, Unnamed: X).
    """
    cols_a_eliminar = []
    for col in df.columns:
        if 'Unnamed' in str(col):
            ...
            continue
        valores_unicos = df[col].dropna().astype(str).str.strip().unique()
        valores_sistema = {'0', '', 'No existen datos para esta consulta.'}
        if len(valores_unicos) == 0 or all(v in valores_sistema for v in valores_unicos):
            cols_a_eliminar.append(col)

    if cols_a_eliminar:
        print(f"   🧹 Columnas extra eliminadas: {cols_a_eliminar}")
        df = df.drop(columns=cols_a_eliminar)
    else:
        print("   ✅ No se detectaron columnas extra")
    return df
...
def limpiar_columnas_id(df):
    ...
    Latitud y Longitud NO se tocan: deben conservar sus decimales.
    """
...
def procesar_json_a_dataframe(json_text):
    ...
    try:
        ...
        # Limpiar columnas fantasma del webservice
        df = limpiar_columna_extra(df)
...
def exportar_datos(df, carpeta_salida, nombre_fijo, guardar_historico=True):
    ...
    try:
        ...
        if guardar_historico:
            ...
            archivo_historico = f"historico_visitas_{timestamp}.csv"
...
if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("  ACTUALIZACIÓN AUTOMÁTICA - REPORTE VISITA A CENTRO DE TRABAJO")
    print("=" * 70)
